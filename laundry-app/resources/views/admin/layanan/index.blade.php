                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 font-semibold">{{ $layanan->nama_layanan }}</td>
                            <td class="px-6 py-4 text-green-600 font-bold">Rp {{ number_format($layanan->tarif, 0, ',', '.') }}</td>
                            <td class="px-6 py-4">{{ $layanan->satuan }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $layanan->deskripsi ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2 justify-center">
                                    <!-- Button Edit -->
                                    <button onclick='editLayanan({{ json_encode($layanan) }})' 
            ...
            <div class="lg:hidden space-y-3">
                ...
                <div class="bg-white rounded-lg shadow-sm p-4">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <div class="font-bold text-sm">{{ $layanan->nama_layanan }}</div>
                            <div class="text-xs text-gray-500 mt-1">{{ $layanan->satuan }}</div>
                        </div>
                        <div class="text-green-600 font-bold text-sm">Rp {{ number_format($layanan->tarif, 0, ',', '.') }}</div>
                    </div>
                    ...
                    <div class="flex justify-end gap-2">
                        <button onclick='editLayanan({{ json_encode($layanan) }})' class="text-blue-600 p-2" title="Edit">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                        </button>
                        <button onclick="deleteLayanan({{ $layanan->id }})" class="text-red-600 p-2" title="Hapus">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                        </button>
